<section class="bg-[#0D0D0D]">
    ...
    <form class="mt-8" enctype="multipart/form-data" method="post" action="{{route('photo.update',$user)}}">
        @csrf
        @method('patch')
        ...
        <div>
            ...
            <input id="photoprofile" name="photoprofile" type="file"
                class="mt-1 block w-full  bg-transparent text-slate-200"
                :value="old('photoprofile', $user->photoprofile)" required autofocus autocomplete="photoprofile" />
            <x-input-error class="mt-2 text-red-600" :messages="$errors->get('photoprofile')" />
        </div>
        <x-primary-button class="mt-6 mb-5">{{ __('Save') }}</x-primary-button>
    </form>
    ...
    <form enctype="multipart/form-data" method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full  bg-transparent text-slate-200"
        ...
        <div>
            ...
            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full bg-transparent text-slate-200"
